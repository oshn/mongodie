...
test('Produce less `exo`s', async t => {
    let die = new MongoDie(t.context.cursor, 'load', 'name');
...
test('Behave fair', async t => {
    let die = new MongoDie(t.context.cursor, null, 'name');
...
test('Take whole entry', async t => {
    let die = new MongoDie(t.context.cursor, 'load', false);

    let re = /hopar|exo|venus/;

    let o = await die.derive();

    t.true(re.test(o.name));
});
